...
/*   Updated: 2022/05/19 23:23:08 by ftegan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_xpm_init(t_so_long	*game)
{
	...
	game->rkiller = mlx_xpm_file_to_image(game->mlx, IMG_RKILLER, &w, &h);
	game->lkiller = mlx_xpm_file_to_image(game->mlx, IMG_LKILLER, &w, &h);
	game->step_label = mlx_xpm_file_to_image(game->mlx, IMG_STEP_LABEL, &w, &h);
}
