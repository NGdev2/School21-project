...
/*   Updated: 2022/05/20 20:36:48 by ftegan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (s_game->j < s_game->lines)
	{
		while (s_game->i < s_game->len_line)
		{
			if (s_game->map[s_game->j][s_game->i] == PLAYER)
				ft_save_pos(s_game, &(s_game->heropos), &s_game->p);
			...
			else if (s_game->map[s_game->j][s_game->i] == KILLER)
				ft_save_pos(s_game, &(s_game->kpos), &s_game->k);
